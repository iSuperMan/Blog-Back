...
schema.methods.likeBy = function (user, callback) {
...
schema.methods.unlikeBy = function (user, callback) {
...
schema.statics.getPopular = function (callback) {
	waterfall([
		cb => this.aggregate([
		...
		(projectStories, cb) => this.find(
			{ _id: { $in: projectStories.map(story => story._id) } },
			...
			(err, stories) => {
				...
				return cb(null, projectStories.map(({ _id }) => _.find(stories, { _id })));
			}),
	], callback);
};
